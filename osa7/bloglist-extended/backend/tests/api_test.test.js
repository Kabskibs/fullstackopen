...
describe('API tests', () => {
  ...
  describe('PUT tests', () => {
    test('updating likes of latest post works', async () => {
      ...
      const lastPostBefore =
        responseBefore.body[responseBefore.body.length - 1];
      const body = {
        likes: 1000,
      };
      await api.put(`/api/blogs/${lastPostBefore.id}`).send(body).expect(201);
    ...
    test('updating nonexistent id returns 404', async () => {
      ...
      await api.put(`/api/blogs/${nonexistentId}`).send(body).expect(404);
    });
    ...
    test('invalid id returns 400', async () => {
      ...
      await api.put(`/api/blogs/${invalidId}`).send(body).expect(400);
    });
  });
...
describe('USERS tests', () => {
  test('user can be created', async () => {
    ...
    assert.strictEqual(
      response.body[response.body.length - 1].username,
      'new_user',
    );
  });
